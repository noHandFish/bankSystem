...
<div style="width: 98%;height: 10%;background-color: white;border-radius: 6px;margin:20px auto">
	...
	<div style="width: 98%;margin: 20px auto;height:80%;display: flex;flex-wrap: wrap; overflow-y: scroll; background-color: white;padding: 20px 0 0 20px; box-sizing: border-box; position: relative;">
		<div style="position: absolute; top: 3%; left: 2%;cursor: pointer;">
<!--			<img src="../imag/pre.png" @click="prePage"  style="width: 20px; height: 20px; ">-->
		</div>
		<div style="margin-top: 2%;">管理类图像</div>
...
<script setup>
	...
	const img = ref([])
	onMounted(()=>{
		if(sessionStorage.getItem("manageImg")){
			img.value = JSON.parse(sessionStorage.getItem("manageImg"))
			console.log(img.value)
			delete img.value.cliIdnum
	...
	function handler(data){
		// sessionStorage.setItem("manageImg",JSON.stringify(data.data.data[0]))
		img.value = JSON.parse(sessionStorage.getItem("manageImg"))
		delete img.value.cliIdnum
		let temp = Object.values(img.value)
		console.log(temp)
