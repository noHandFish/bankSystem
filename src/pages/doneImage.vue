...
<!--			<img src="../imag/pre.png" @click="prePage"  style="width: 20px; height: 20px; cursor: pointer;">-->
		...
		<div style="font-size: 18px;font-family: Source Han Sans CN, Source Han Sans CN-Bold;font-weight: 700;text-align: left;color: #32323a; margin-top: 2%;">已结清业务</div>
...
<script setup>
	...
	const img = ref([])
	onMounted(()=>{
		if(sessionStorage.getItem("doneImg")){
			img.value = JSON.parse(sessionStorage.getItem("doneImg"))
	...
	function handler(data){
		// sessionStorage.setItem("doneImg",JSON.stringify(data.data.data[0]))
		img.value = JSON.parse(sessionStorage.getItem("doneImg"))
